...
const GradientRule2 = ({ data }) => {
  ...
  function calculateLeftStyle(result, data_up_boundary, data_down_boundary) {
    ...
    if (result >= l && result <= data_up_boundary) {
      return (
        (((result - l) / (data_up_boundary - l)) * 125 - 90).toString() + "%"
      );
    }
    ...
    if (result >= data_down_boundary && result <= r) {
      return (
        (
          ((result - data_down_boundary) / (r - data_down_boundary)) * 105 +
          315
        ).toString() + "%"
      );
    }
      ...
      >
        ...
        >
          ...
          <View style={styles.separatorContainer}>
            <View style={[styles.separator, { left: "-25%" }]}>
              <Text style={styles.separatorText}>
                {colorBar.data_up_boundary.toFixed(2)}
              </Text>
            </View>
            <View style={[styles.separator1, { left: "500%" }]}>
              <Text style={styles.separatorText}>
