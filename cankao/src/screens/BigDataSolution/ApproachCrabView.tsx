...
const ApproachCrabView: React.FC<{
  minY: number;
  maxY: number;
  minX: number;
  maxX: number;
  ...
  const calculateImageBackgroundStyle = () => {
    const rangeX = maxX - minX; // X轴范围
    const rangeY = maxY - minY; // Y轴范围
    ...
    const offsetX = (chartWidth / 5) * 3 + padding; // x偏移量
    ...
    return {
      ...
      height: imageHeight,
      top: offsetY, // 顶部偏移
      left: offsetX, // 设置左侧偏移,由于斜视的跑道图被人为拉伸，左侧多出了30像素的空间，所以要减去
...
export default ApproachCrabView;
