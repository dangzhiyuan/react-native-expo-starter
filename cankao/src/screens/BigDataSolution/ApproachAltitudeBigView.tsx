...
import { getTargetDataPointsScatter, parseDateTime } from "./tools";

export type DomainTuple = [Date, Date];

const ApproachAltitudeBigView: React.FC<{
  minY: number;
  maxY: number;
  gapY: number;
  minX: number;
  ...
  const calculateImageBackgroundStyle = () => {
    ...
    const imageWidth = (runway.rwy_length * (chartWidth * 0.93)) / maxX; // 跑道宽度(NM)，坐标维度
    ...
    const offsetX = (chartWidth / 5) * 4 + padding; // x偏移量
    ...
    return {
      ...
      height: imageHeight,
      top: offsetY, // 顶部偏移
      left: offsetX, // 设置左侧偏移,由于斜视的跑道图被人为拉伸，左侧多出了50像素的空间，所以要减去
        ...
        >
          ...
          >
            ...
            <VictoryAxis
              ...
              tickFormat={(x) =>
                x < 0 ? -parseFloat(x).toFixed(0) : parseFloat(x).toFixed(0)
              }
              label={xAxis}
              offsetY={50}
              style={{
...
export default ApproachAltitudeBigView;
