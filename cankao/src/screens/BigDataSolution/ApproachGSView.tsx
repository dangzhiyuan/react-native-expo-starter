...
const ApproachGSView: React.FC<{
  ...
  console.log("originalData:", tdOffset);
  const currentRef = "gs_view";
  ...
  const flareEndItems = filterDataPoints(originalData.filter((item)=>item.value.loc_touch_down===true).map((item)=>{
  ...
  const padding = 50;
  // const minXtmp = Math.min(...xValues);
  // const maxXTmp = Math.max(...xValues);
  // const minYtmp = Math.min(...yValues);
  // const maxYtmp = Math.max(...yValues);
  ...
  const customData = [
    { x: minX, y: renderedDataTmp[0].value["ref"] },
    { x: tdOffset, y: 0 },
  ];
  ...
  const calculateImageBackgroundStyle = () => {
    ...
    const radians = (0.8 * Math.PI) / 180; // Convert 0.8 degrees to radians

    const tanRadians = Math.tan(radians);
    if (tanRadians === 0) {
      return { width: 0, height: 0, top: 0, left: 0 };
    }

    const imageHeight =
      (runway.rwy_width * chartHeight) /
      (2 * Math.abs(minX) * 1852 * tanRadians) /
      4;
    ...
    let offsetX = (chartWidth * Math.abs(minX)) / rangeX + padding - 8;
    if (minX > 0) {
      offsetX = -(chartWidth * Math.abs(minX)) / rangeX + padding - 8;
    }
    const offsetY = padding - imageHeight / 2 + 100;

    return {
      width: (chartWidth * maxX) / rangeX,
      height: imageHeight,
      top: offsetY,
      left: offsetX,
          ...
          >
            ...
            {tolUpper !== 99999.9 && (
              <VictoryLine
                ...
                x={(d) => d.value.x}
                y={(d) =>
                  d.value.ref !== null ? d.value.ref + tolUpper / 2 : null
                }
              />
            )}
            ...
            {tolLower !== 99999.9 && (
              <VictoryLine
                ...
                x={(d) => d.value.x}
                y={(d) =>
                  d.value.ref !== null ? d.value.ref + tolLower / 2 : null
                }
              />
            )}
            ...
            {flareEndItems && flareEndItems.length > 0 && (
              <VictoryLine
                data={[
                  {
