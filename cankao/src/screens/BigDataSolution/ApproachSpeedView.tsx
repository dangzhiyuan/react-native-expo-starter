...
const ApproachSpeedView: React.FC<{
  minY: number;
  maxY: number;
  minX: number;
  maxX: number;
  ...
  // const originalData = rdpSimplify(data, 0.6);
  const currentRef = "speed_with_vapp_view";
  ...
  const calculateDy = (data, index) => {
    ...
    if (prevY !== null && Math.abs(data[index].x - data[index - 1].x) < 0.5) {
      if (currentY > prevY) {
        return 100;
    ...
    <PaperProvider>
      <Portal>
        ...
        >
          ...
          {/* <Image
            source={require("../../../assets/images/runway_ios_01.png")}
            style={{
              position: "absolute",
              width: "73%",
              height: "35%",
...
export default ApproachSpeedView;
