...
const TrackTopView: React.FC<{
  ...
  const currentRef = "ex_top_view";
  const imagePath = getImagePath(runway.ap_icao + runway.rwy_id, "top");
  const flareStartItems = originalData.filter(
    (item) => item.value.loc_lift_off === true
  );
  console.log("flareStartItems:", flareStartItems);
  // const originalData = rdpSimplify(
  //   data.map((d) => ({ x: d.value.x, y: d.value.y })),
  //   0.00001
  // );

  // console.log("偏离:", originalData);
  ...
  const chartWidth = screenWidth - padding * 2.5; // 计算视图（图表）的实际宽度
  ...
  const calculateImageBackgroundStyle = () => {
    ...
    const imageWidth = (chartWidth * 0.97 * runway.rwy_length) / 1.7 / rangeX;
    ...
    return {
      width: (1500 / 1852 / maxX) * 588 * 0.95, //除以2的意思是横轴最大2海里，2×1852是横轴实际多少米
      height: imageHeight,
        ...
        >
          ...
          >
            ...
            >
              ...
              {flareStartItems && flareStartItems.length > 0 && (
                <VictoryLine
                  data={[
                    {
