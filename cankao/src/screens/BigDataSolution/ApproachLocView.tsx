...
const ApproachLocView: React.FC<{
  minY: number;
  maxY: number;
  minX: number;
  maxX: number;
  ...
  const currentRef = "loc_view";
  const imagePath = getImagePath(runway.ap_icao + runway.rwy_id, "top");
  const flareEndItems = originalData
    .filter((item) => item.value.loc_touch_down === true)
    .map((item) => {
...
export default ApproachLocView;
