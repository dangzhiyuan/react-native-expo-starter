...
const ApproachDeviationView: React.FC<{
  minY: number;
  maxY: number;
  minX: number;
  maxX: number;
  ...
  const angle3 =
    (tdOffset + Math.abs(minX)) * 1852 * Math.tan((0.8 * Math.PI) / 180);

  const angle3Top =
    tolUpper !== 99999.9
      ? (tdOffset + Math.abs(minX)) *
        1852 *
        Math.tan((tolUpper * Math.PI) / 180)
      : null;
  ...
  const angle3TopHalf =
    tolUpper !== 99999.9
      ? (tdOffset + Math.abs(minX)) *
        1852 *
        Math.tan(((tolUpper / 2) * Math.PI) / 180)
      : null;
  ...
  const angle3DownHalf =
    tolLower !== 99999.9
      ? (tdOffset + Math.abs(minX)) *
        1852 *
        Math.tan(((tolLower / 2) * Math.PI) / 180)
      : null;
  ...
  const calculateImageBackgroundStyle = () => {
    ...
    let offsetX = (chartWidth * Math.abs(minX)) / rangeX + padding - 22;
    if (minX > 0) {
      offsetX = -(chartWidth * Math.abs(minX)) / rangeX + padding - 22;
    }
...
export default ApproachDeviationView;
