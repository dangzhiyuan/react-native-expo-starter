...
const TakeOffDeviationView: React.FC<{
  minY: number;
  maxY: number;
  minX: number;
  maxX: number;
  ...
  const flareStartItems = originalData
    .filter((item) => item.value.loc_lift_off === true)
    .map((item) => {
      const [hours, minutes, seconds] = item.time.split(":");
      const newTime = `${hours}:${minutes}:${seconds.split(".")[0]}`;
      return { ...item, time: newTime };
    });
  console.log("flareStartItems:", originalData);
  ...
  const calculateImageBackgroundStyle = () => {
    ...
    console.log("imageHeight", imageHeight, rangeY);
    ...
    return {
      width: (1500 / 1852 / maxX) * 570,
      height: imageHeight,
      top: offsetY - 16, // 顶部偏移，padding为当前设备默认chart顶部距离外部view顶部的距离，100为当前chart区域高度的一半
...
export default TakeOffDeviationView;
