...
const ApproachTouchdownGlideView: React.FC<{
  minY: number;
  maxY: number;
  minX: number;
  maxX: number;
  ...
  const currentRef = "till_touchdown_side_view";
  ...
  const calculateImageBackgroundStyle = () => {
    ...
    const imageWidth = (chartWidth * 3336) / 1852 / rangeX;
    ...
    let offsetX = (chartWidth * Math.abs(minX)) / rangeX - 45;
    if (minX > 0) {
    ...
    return {
      width: (1.6 * (chartWidth * maxX)) / rangeX,
      height: imageHeight,
      top: offsetY, // 顶部偏移
      left: offsetX + 65, // 设置左侧偏移,由于斜视的跑道图被人为拉伸，左侧多出了30像素的空间，所以要减去
...
export default ApproachTouchdownGlideView;
