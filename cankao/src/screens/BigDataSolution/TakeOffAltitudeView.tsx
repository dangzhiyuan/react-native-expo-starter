...
const TakeOffAltitudeView: React.FC<{
  minY: number;
  maxY: number;
  minX: number;
  maxX: number;
  ...
  const padding = 50;
  // const minXT = Math.min(...xValues);
  // const maxXT = Math.max(...xValues);
  // const minYT = Math.min(...yValues);
  // const maxYT = Math.max(...yValues);
  ...
  const getData = useCallback((): DataPoint[] => {
    ...
    return [];
  }, [originalData, zoomedXDomain, maxPoints]);

  useEffect(() => {
    if (originalData) {
      setMaxPoints(originalData.length);
      getData();
    }
  }, [originalData]);
  ...
  console.log("processedData data", renderedDataTmp);
  ...
  const calculateImageBackgroundStyle = () => {
    ...
    const rangeX = maxX - minX; // X轴坐标范围
    const rangeY = maxY - minY; // Y轴坐标范围
    console.log("range:", rangeX, rangeY);
    const imageWidth = (chartWidth * runway.rwy_length) / rangeX;
    const imageHeight =
      (runway.rwy_width * (chartHeight - padding * 2)) / rangeY; // 跑道高度(m)，坐标维度
    if (!isFinite(imageWidth) || !isFinite(imageHeight)) {
      return { width: 0, height: 0, top: 0, left: 0 };
    }
    console.log("imageWidth:", imageWidth, imageHeight);
    const offsetX = chartWidth * (minX / rangeX) + padding; // x偏移量
    const offsetY = padding - imageHeight / 2 + 200; // y偏移量
    console.log("offsetX:", offsetX);
    return {
      width: (1500 / 1852 / maxX) * 588, //由于当前跑道图被拉伸为斜视，为了美观额外增加70宽度
      height: imageHeight, //由于当前跑道图被拉伸为斜视，为了美观额外增加50宽度
      top: offsetY, // 顶部偏移，padding为当前设备默认chart顶部距离外部view顶部的距离，跑道图高度被额外增加了50也要算进去，200为当前chart区域高度
      left: offsetX - 18, // 设置左侧偏移，跑道斜视，微调
...
export default TakeOffAltitudeView;
